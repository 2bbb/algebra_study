...
namespace bbb {
    namespace math {
        ...
        struct polynomial {
            ...
            static polynomial monomial(cofficient_field_type a, std::size_t degree) {
                std::vector<cofficient_field_type> coef;
            ...
            polynomial(const std::vector<cofficient_field_type> &coef)
            : coef{coef}
            {};
            
            polynomial(std::vector<cofficient_field_type> &&coef)
            : coef{std::move(coef)}
            {};
            ...
            template <
                ...
                typename = typename std::enable_if<
                    !std::is_same<utility::remove_const_reference<X>, polynomial>::value
                    && !std::is_same<utility::remove_const_reference<X>, std::vector<cofficient_field_type>>::value
                >::type
            >
            polynomial(X && v, Xs && ... vs)
            : coef{{cofficient_field_type{std::forward<X>(v)}, cofficient_field_type{std::forward<Xs>(vs)} ...}}
            {};
    ...
    #pragma mark /
            ...
            inline friend polynomial operator/(cofficient_field_type scalar, const polynomial &rhs)
    ...
    #pragma mark compare
            ...
            cofficient_field_type &leading_coefficient()
            { return coef.back(); };
            cofficient_field_type leading_coefficient() const
            { return coef.back(); };
            
            cofficient_field_type &operator[](std::size_t index)
            { return coef.at(index); };
            cofficient_field_type operator[](std::size_t index) const
            { return coef.at(index); };
            
            cofficient_field_type &at(std::size_t index)
            { return coef.at(index); };
            cofficient_field_type at(std::size_t index) const
            { return coef.at(index); };
